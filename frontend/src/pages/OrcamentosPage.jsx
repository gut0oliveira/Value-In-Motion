...
import { buscarCategorias, buscarTransacoes } from "../lib/api";

const STORAGE_KEY = "vimo_orcamentos";
...
export default function OrcamentosPage() {
  ...
  useEffect(() => {
    async function carregarBase() {
      try {
        const [categoriasApi, transacoesApi] = await Promise.all([buscarCategorias(), buscarTransacoes()]);
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (Array.isArray(dados)) setOrcamentos(dados);
    } catch {
  ...
  function salvarOrcamento(evento) {
    ...
    if (editandoId) {
      setOrcamentos((atual) => atual.map((item) => (item.id === editandoId ? payload : item)));
    } else {
      setOrcamentos((atual) => [payload, ...atual]);
    }
    limparFormulario();
    setMostrarFormulario(false);
  }

  async function excluirOrcamento(item) {
