...
export default function MetasPage() {
  ...
  function salvarMeta(evento) {
    ...
    const payload = {
      ...
      created_at: editandoId ? metas.find((item) => item.id === editandoId)?.created_at || hojeIso() : hojeIso(),
    };

    if (editandoId) {
      setMetas((atual) => atual.map((item) => (item.id === editandoId ? payload : item)));
    } else {
      setMetas((atual) => [payload, ...atual]);
    }
    limparFormulario();
    setMostrarFormulario(false);
  }

  async function excluirMeta(item) {
  ...
  function alternarStatusMeta(item) {
    setMetas((atual) =>
      atual.map((meta) =>
        meta.id === item.id
          ? {
              ...meta,
              active: item.status === "completed" ? meta.active : !meta.active,
            }
          : meta
      )
    );
  ...
  function concluirMeta(item) {
    setMetas((atual) =>
      atual.map((meta) =>
        meta.id === item.id
          ? {
              ...meta,
              current_amount: Number(meta.target_amount),
              active: true,
            }
          : meta
      )
